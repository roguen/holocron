...
| Feedback | **A crystal can sample the frame it drew last** — `feedback = true` in the manifest, then `u_feedback` and `u_has_feedback` in the shader (issue 373, **D-079**). This was the first entry under *what a crystal cannot do yet* and `docs/cutting-crystals.md` called it **the single biggest limit versus the MilkDrop lineage**. **Ping-pong, not a copy**: the layer gets a second target, `swap_feedback` exchanges them AFTER the composite has read the new one, and swapping first would draw one frame stale with no other symptom. **Allocated only when asked** — 66 MB at 4K — the same rule the read-back canvas follows, and asked for EVERY frame rather than once, because a surface can go away underneath a crystal. **Not on `TrackContext`**, where the art lives: art belongs to what is playing, feedback belongs to the LAYER, and on the track two crystals in an archive would share one history. The decay constant is the whole stability of such a shader — at or above 1.0 it diverges to white, slowly enough that 0.995 looks fine for ten seconds. |
...
Current version `v1.0.6`. `main` is stable and CI is green. Bump **in the same
...
than a forgotten field. `versionCode` is **15** at `v1.0.6`. Plus the wiki's Home
